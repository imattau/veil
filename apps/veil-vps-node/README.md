...
- `VEIL_VPS_BLE_ENABLED` (`true`/`false` or `1`/`0`, default `false`, requires `--features ble-btleplug`)
...
- `VEIL_VPS_SNAPSHOT_INTERVAL` (default `60s`, human-readable duration)
- `VEIL_VPS_TICK_INTERVAL` (default `50ms`, human-readable duration)
...
- `VEIL_VPS_ADAPTIVE_LANE_SCORING` (`true`/`false` or `1`/`0`, default `true`)
- `VEIL_VPS_PROBABILISTIC_FORWARDING` (`true`/`false` or `1`/`0`, default `true`)
- `VEIL_VPS_FORWARDING_MIN_PROBABILITY` (default `0.10`)
- `VEIL_VPS_FORWARDING_REPLICA_DIVISOR` (default `8`)
- `VEIL_VPS_BLOOM_EXCHANGE` (`true`/`false` or `1`/`0`, default `true`)
- `VEIL_VPS_BLOOM_INTERVAL_STEPS` (default `128`)
- `VEIL_VPS_BLOOM_FALSE_POSITIVE_RATE` (default `0.05`)
- `VEIL_VPS_OPEN_RELAY` (`true`/`false` or `1`/`0`, default `false`; accept all tags and forward all non-blocked peers)
- `VEIL_VPS_BLOCKED_PEERS` (comma-separated peer ids to hard-block, e.g. `1.2.3.4:5000,ws:relay-a`)
- `VEIL_VPS_NOSTR_BRIDGE_ENABLED` (`true`/`false` or `1`/`0`, default `false`)
- `VEIL_VPS_NOSTR_BRIDGE_RELAYS` (comma-separated relay URLs, e.g. `wss://relay.damus.io,wss://nos.lol`)
- `VEIL_VPS_NOSTR_BRIDGE_CHANNEL_ID` (default `nostr-bridge`)
- `VEIL_VPS_NOSTR_BRIDGE_NAMESPACE` (default `32`)
- `VEIL_VPS_NOSTR_BRIDGE_SINCE` (default `1h`, human-readable duration)
- `VEIL_VPS_NOSTR_BRIDGE_STATE_PATH` (default `data/nostr-bridge-state.json`)
- `VEIL_VPS_NOSTR_BRIDGE_MAX_SEEN_IDS` (default `20000`)
- `VEIL_VPS_NOSTR_BRIDGE_PERSIST_EVERY_UPDATES` (default `32`)

## Notes
- Configuration is loaded from environment variables and an optional `--config` TOML file.
- SQLite settings (`data/settings.db`) are primarily for the web dashboard.
- All time-based settings support human-readable durations (e.g., `10s`, `5m`, `1h`).
- List-based settings (like peers and tags) can be provided as comma-separated strings in environment variables.
